...
def get_args(parser):
    ...
    parser.add_argument("--model", type=str, default="bow", choices=["bow", "img", "bert", "concatbow", "concatbert", "mmbt", "concatbowrelu", "concatbowbnrelu"])
...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    if args.model == "bow":
        ...
        img = img.cuda()
        out = model(img)
    elif args.model == "concatbow" or args.model == "concatbowrelu" or args.model == "concatbowbnrelu":
...
def train(args):
    ...
    load_checkpoint(model, os.path.join(args.savedir, "model_best_1.pt"))
    model.eval()
