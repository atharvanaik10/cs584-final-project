...
class SimplexLP(LinearizedNetwork):
    ''' // 实现了论文中提出的 Simplex Verify 算法，这是一种针对我们提出的松弛的快速lirpa式算法。
    The objects of this class are s.t: the input lies in l1.
    1. the first layer is conditioned s.t that the input lies in a probability simplex  //1st层的条件: 输入位于probability simplex中
    2. Simplex Propagation: the layers(including first layer, excluding output linear layer) are conditioned s.t their output also lies in simplex, this is done using the simplex cut.
                          // 各层(包括第一层,不包括output linear层) 的条件是它们的输出也位于simplex中, 这是通过simplex cut完成的
    3. better ib bounds are computed using these propagated simplices    //使用这些propagated的simplex计算更好的 ib bound
    ...
    // 返回：因此, define_linear_approximation fn 将返回一个输入位于simplex内, 且所有其他中间层也位于simplex内的网络。
    '''
    def __init__(self, 
        layers, 
        debug=False, 
        ...
        store_bounds_progress=-1, # 是否随着时间存储bounds progress
        store_bounds_primal=False, # 是否随存储 用于计算最终bounds的 原始解primal solution
        max_batch=20000,    # 在输出神经元和domains上 并行bounding计算 的最大batch大小
        seed=0,
        dp=True,
        tgt=1):
        """
        ...
        self.layers = layers
        self.net = nn.Sequential(*layers)   # 定义net
    ...
    def set_decomposition(self, decomp_style, decomp_args, ext_init=None):  # 设置decomposition
        decompositions = {
    ...
    def set_solution_optimizer(self, method, method_args=None):     # 设置答案优化器
    ...
    def compute_lower_bound(self, node=(-1, None), upper_bound=False, counterexample_verification=False):
        ...
        '''
        additional_coeffs = {}  ## 
        ### self.lower_bounds =【（tensor shape,  lower bound,  preactivation bound）， ～～～】
        current_lbs = self.lower_bounds[node[0]]    ### 获取"node"层的tensor shape
        if current_lbs.dim() == 0:
            current_lbs = current_lbs.unsqueeze(0)
        node_layer_shape = current_lbs.shape[1:]    ### 获取输出形状

        batch_size = current_lbs.shape[0]   ### 用这个layer的shape 得到batch_size
        ...
        # Optimize all the bounds       # 在批处理的情况下，我们无论如何都需要对所有层进行优化，否则不同大小的tensors应保存为一个列表（慢速）
        ...
        ### 如果对输出神经元边界进行并行化处理后得出的批次规模过大，我们需要将其划分为多个子批次
        neuron_batch_size = nb_out * 2 if is_batch else 1
        c_batch_size = int(math.floor(self.max_batch / batch_size))     ### c_batch_size是每个子批次里能塞的神经元个数
        n_batches = int(math.ceil(neuron_batch_size / float(c_batch_size)))     ### n_batches是一共要分多少批
        print(f"----------------> {c_batch_size} * {n_batches}; total {neuron_batch_size}*{batch_size}")

        bound = None
        ### 遍历每个子批次进行绑定优化
        #   在batching什么？？？？？？？ 应该是batching一个layer
        for sub_batch_idx in range(n_batches):
            ### 从这个sub-batch开始计算 intermediate bounds； 用index确定sub-batch范围
            start_batch_index = sub_batch_idx * c_batch_size
            end_batch_index = min((sub_batch_idx + 1) * c_batch_size, neuron_batch_size)

            ### slice_coeffs表示从哪个输出神经元开始，到哪一个神经元结束  【没有被使用】
            ...
            ### additional_coeffs就是优化器的目标函数约束      【没有被使用】
            additional_coeffs[lay_to_opt] = slice_coeffs

            ########################################################################
            ###### 这里使用了auto_lirpa_optimizer函数来得倒model的最优 输出bounds
                # 它进行多次迭代，对weight进行优化；每次迭代都循环所有layer来得到输出bounds
                # 最后它不会储存优化过的weigth系数，只会返回：在最优weigtht系数的场合 计算出的model的最优输出bounds
            c_bound = self.optimize(self.weights, additional_coeffs, self.lower_bounds, self.upper_bounds)

            ### 将返回结果c_bound加入到bound内
                # 返回一个[batch_size, 2*num_neurons]是的矩阵（前半是-upper_bound，后半是lower_bound）
    ...
    def define_linear_approximation(self, input_domain, emb_layer=False, no_conv=False, override_numerical_errors=False):
        ...
        for lay_idx, layer in enumerate(self.layers):
            ...
            if lay_idx == 0:
                ...
                if not emb_layer:       #【tensor shape,  lower bound,  preactivation bound】
                ...
                layer_opt_start_time = time.time()          ### 调用self.optimize
    ...
    def build_model_using_intermediate_net(self, domain, intermediate_bounds, intermediate_net, no_conv=False):
        ...
        根据提供的中间bounds建立model
            - 如果no_conv为true, conv层会被视为等效的linear层。在这种情况下, 所提供的中间bounds应保留conv结构
        """
        self.no_conv = no_conv
        self.input_domain = domain
        ref_lbs, ref_ubs = copy.deepcopy(intermediate_bounds)   # 复制所有的中间层bounds
        int_weights = copy.deepcopy(intermediate_net.weights)   # 复制未优化的weight
        ...
        # Add the first layer, appropriately rescaled.  取出第一个layer的weight
        self.weights = [int_weights[0]]
        # Change the lower bounds and upper bounds corresponding to the inputs
        if not no_conv:     ### 如果不是conv layer，就将中间层bounds 作为self.lower_bounds和self.upper_bounds
        ...
        for layer in self.layers[1:]:   
            ### 循环除了第一层外的所有层，如果是Conv或linear层，就将对应层的“未优化的weight”加到self.weights内
    ...
    def auto_lirpa_optimizer(self, weights, final_coeffs, lower_bounds, upper_bounds):
        ...
        ### 更新这个object内的 self.lower_a和self.upper_a， 即优化weight，更新weight 
            # 返回：最后一次weight系数迭代优化时 计算出的bound                                                               dp=True
