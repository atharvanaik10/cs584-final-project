...
import math
import copy
from plnn.simplex_solver import baseline_anderson_optimization, baseline_bigm_optimization
...
def compute_dual_subgradient_adam(weights, clbs, cubs, nubs, dual_vars, primal_vars, steps, precision=torch.float,
    ...
    for lay_idx in range(nb_relu_layers):
        ...
        if lay_idx == 0:
            # x0, _ = CutPrimalVars.first_layer_primalsk_min(lay_idx, dual_vars, clbs, cubs, precision)
...
class CutDualVars(baseline_anderson_optimization.DualVars):
    """
    ...
    def naive_initialization(weights, additional_coeffs, device, input_size, alpha_M=None, beta_M=None):
        ...
        """
        base_duals = baseline_anderson_optimization.DualVars.naive_initialization(weights, additional_coeffs, device, input_size)
    ...
    def alphak_grad(self, lay_idx, weights, primal_vars, precision):
        ...
        compute and return the gradient of alpha_k with new alpha_k.
        """
    ...
    def betak_grad_lmo(self, lay_idx, weights, clbs, cubs, nubs, primal_vars, outer_it, opt_args):
        """
        Given list of layers, primal anchor points (instance of SaddlePrimalVars), pre and post activation bounds,
        compute and return the linear minimization oracle of beta_k with its gradient, and the gradient itself.
    ...
    def update_from_step(self, weights, dual_vars_subg, lay_idx="all"):
        ...
        for lay_idx in lay_to_iter:
            if lay_idx > 0:
                ...
                self.update_duals_from_betak(lay_idx, weights, dual_vars_subg.sum_beta[lay_idx], dual_vars_subg.sum_Wp1Ibetap1[lay_idx-1], dual_vars_subg.sum_W1mIubeta[lay_idx], dual_vars_subg.sum_WIlbeta[lay_idx], dual_vars_subg.beta_M[lay_idx])
...
class DualADAMStats:
    ...
    def update_moments_take_projected_step(self, weights, step_size, outer_it, dual_vars, dual_vars_subg, primal_vars,  clbs, cubs, nubs, l_preacts, u_preacts, cut_frequency, max_cuts, precision, opt_args):
        """
        ...
        for lay_idx in range(1, len(dual_vars.sum_beta)):
            ...
            if type(lin_k) in [utils.ConvOp, utils.BatchConvOp]:
                masked_op = baseline_anderson_optimization.MaskedConvOp(lin_k, xkm1, dual_vars.sum_beta[lay_idx])
                ...
                masked_op = baseline_anderson_optimization.MaskedLinearOp(lin_k)
...
class CutPrimalVars(baseline_anderson_optimization.PrimalVars):
    ...
    def mid_box_initialization(dual_vars, clbs, cubs):
        ...
        """
        primals = baseline_anderson_optimization.PrimalVars.mid_box_initialization(dual_vars, clbs, cubs)
    ...
    @staticmethod
    def first_layer_primalsk_min(lay_idx, dual_vars, clbs, cubs, precision):
        """
        Given list of post-activation bounds and çurrent dual variables (instance of SaddleDualVars),
        compute and return the linear minimization oracle of x_k and z_k with their gradients, and the gradients
        themselves. (done jointly as they are independent, so no Gauss-Seidel effect)
...
class CutInit(baseline_anderson_optimization.AndersonPInit):
    """
    ...
    def as_stack(self, stack_size):
        ...
        for varset in [self.primals.xt, self.primals.zt]:
            stacked_primal_list.append(
                [pinits[0].unsqueeze(0).repeat(((stack_size,) + (1,) * (pinits.dim() - 1))) for pinits in varset])
        return CutInit(baseline_bigm_optimization.DualVars(*stacked_dual_list), CutPrimalVars(*stacked_primal_list))
