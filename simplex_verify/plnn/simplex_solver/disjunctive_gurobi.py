...
class DP_LinearizedNetwork(NaiveNetwork):
    '''对网络进行建议的Simplex Relaxation relaxatio弛，并使用商业求解器计算模型的output bounds.
    In this class. we take the intermediate bounds from SimplexLP and bigm-adam-simplex solver. This means that starting
    from simplex at input, it is propagated and the network is conditioned. 
    This conditioned network is what we create in the build_model_using_bounds function.
    '''
    ...
    def compute_lower_bound(self, node=(-1, None), upper_bound=False, counterexample_verification=False, time_limit_per_layer=None, ub_only=False):
        '''
        ...
        device = self.input_domain[0].device
        ...
        if time_limits is not None:
            ...
            # algorithm. This line of code imposes that.    强制Gurobi使用dual simplex算法。这行代码就强制了这一点
        ...
        if not is_batch:
            ...
            # We will now compute the requested lower bound ################## 计算lower bound
    ...
    def build_model_using_bounds(self, input_domain, intermediate_bounds, init_cut_coeffs=None, n_threads=1):
        """
        ...
        for layer in self.layers:
            ...
            if type(layer) is nn.Linear:
                ...
                for neuron_idx in range(layer.weight.size(0)):
                    if not self.model_built:
                        if x_idx == 1:
                            lin_expr = self.weights[0].bias[neuron_idx].item()
                            coeffs = self.weights[0].weights.squeeze(0)[neuron_idx, :]
                        else:
                            lin_expr = self.weights[x_idx-1].bias[neuron_idx].item()
                            coeffs = self.weights[x_idx-1].weights[neuron_idx, :]
                        ####
                        ## FOR DP CUT ##
                        w_bar = coeffs + lin_expr
                        rw_bar = w_bar
                        for el in range(len(rw_bar)):
                            if rw_bar[el] < 0:
                                rw_bar[el] = 0
                        relu_b = copy.deepcopy(lin_expr)
                        if relu_b < 0:
                            relu_b = 0
                        rw_bar = rw_bar-relu_b
                        dp_cut_expr = relu_b + grb.LinExpr(rw_bar, pre_vars)
                        ####
                    ...
                    dp_cut_expr_vars.append(dp_cut_expr)
                ...
                pre_lbs = self.lower_bounds[x_idx-1].unsqueeze(0)# now it will be (1,3,32,32)
                ...
                for out_chan_idx in range(out_lbs.size(1)):
                    out_chan_vars = [] #all variables corresponding to one output channel
                    dp_chan_vars = []
                    for out_row_idx in range(out_lbs.size(2)):
                        out_row_vars = [] #all variables corresponding to one output row
                        dp_row_vars = []
                        for out_col_idx in range(out_lbs.size(3)):
                            ...
                            if not self.model_built:
                                ...
                                if x_idx==1:
                                    lin_expr = copy.deepcopy(self.weights[x_idx-1].bias[0][out_chan_idx][out_row_idx][out_col_idx].item())
                                    bs = copy.deepcopy(self.weights[x_idx-1].bias[0][out_chan_idx][out_row_idx][out_col_idx].item())
                                    relu_b = copy.deepcopy(self.weights[x_idx-1].bias[0][out_chan_idx][out_row_idx][out_col_idx].item())
                                else:
                                    lin_expr = copy.deepcopy(self.weights[x_idx-1].bias[out_chan_idx].item())
                                    bs = copy.deepcopy(self.weights[x_idx-1].bias[out_chan_idx].item())
                                    relu_b = copy.deepcopy(self.weights[x_idx-1].bias[out_chan_idx].item())
                                ...
                                if relu_b<0:
                                    relu_b=0
                                dp_cut_expr = copy.deepcopy(relu_b)
                                ...
                                for in_chan_idx in range(layer.weight.shape[1]):
                                    for ker_row_idx in range(layer.weight.shape[2]):
                                        ...
                                        for ker_col_idx in range(layer.weight.shape[3]):
                                            ...
                                            if x_idx==1:
                                                if (in_col_idx < 0) or (2*in_col_idx+1 >= pre_lbs.size(3)):
                                                    # This is padding -> value of 0
                                                    continue
                                            else:
                                                if (in_col_idx < 0) or (in_col_idx >= pre_lbs.size(3)):
                                                    ...
                                                    continue
                                            #####    
                                            coeff = self.weights[x_idx-1].weights[out_chan_idx, in_chan_idx, ker_row_idx, ker_col_idx].item()
                                            #####
                                            ...
                                            if x_idx==1:
                                                ...
                                                if w_bar<0:
                                                    w_bar=0
                                                w_neg_bar = -eps*coeff + bs
                                                if w_neg_bar<0:
                                                    w_neg_bar=0
                                                dp_cut_expr += self.gurobi_x_vars[x_idx - 1][in_chan_idx][in_row_idx][in_col_idx*2]*(w_bar-relu_b) + self.gurobi_x_vars[x_idx - 1][in_chan_idx][in_row_idx][in_col_idx*2+1]*(w_neg_bar-relu_b)
                                            else:
                                                w_bar = coeff + bs
                                                if w_bar<0:
                                                    w_bar=0
                                                dp_cut_expr += self.gurobi_x_vars[x_idx - 1][in_chan_idx][in_row_idx][in_col_idx]*(w_bar-relu_b)
                                            ####

                                            if x_idx==1:
                                                lin_expr += eps*coeff * (self.gurobi_x_vars[x_idx - 1][in_chan_idx][in_row_idx][in_col_idx*2] -self.gurobi_x_vars[x_idx - 1][in_chan_idx][in_row_idx][in_col_idx*2+1] ) 
                                            else:
                                                lin_expr += coeff * self.gurobi_x_vars[x_idx - 1][in_chan_idx][in_row_idx][in_col_idx]
                            ...
                            dp_row_vars.append(dp_cut_expr)
                        out_chan_vars.append(out_row_vars)
                        dp_chan_vars.append(dp_row_vars)
                    new_layer_gurobi_vars.append(out_chan_vars)
                    dp_cut_expr_vars.append(dp_chan_vars)
                ...
                if isinstance(self.prerelu_gurobi_vars[x_idx][0], list):
                        ...
                        for row_idx, row in enumerate(channel):
                            ...
                            for col_idx, pre_var in enumerate(row):
                                ...
                                if pre_lb >= 0 and pre_ub >= 0:
                                    ...
                                    #### DP
                                    low_constraint = self.model.addConstr(v >= pre_var)
                                    upp_constraint = self.model.addConstr(v <= dp_cut_expr_vars[chan_idx][row_idx][col_idx])
                                    self.active_planet_constraints.extend([upp_constraint])
                                    ####
                                row_vars.append(v)
                            chan_vars.append(row_vars)
                        if not self.model_built:
                            new_layer_gurobi_vars.append(chan_vars)
                    self.model.addConstr(grb.quicksum(abs_vars) <= 1)
                else:
                    ...
                    for neuron_idx, pre_var in enumerate(self.prerelu_gurobi_vars[x_idx]):
                        ...
                        if pre_lb >= 0 and pre_ub >= 0:
                            ...
                            ##### DP
                            low_constraint = self.model.addConstr(v >= pre_var)
                            upp_constraint = self.model.addConstr(v <= dp_cut_expr_vars[neuron_idx])
                            self.active_planet_constraints.extend([upp_constraint])
                            #####
                        new_layer_gurobi_vars.append(v)
                    self.model.addConstr(grb.quicksum(abs_vars) <= 1)
        ...
        if X.dim() == 2:
            ...
            for dim in range(X.size(0)):
                ...
                if not self.model_built:
                    ...
                    ##
                    ## modelling l1 constraint 
                    # by using auxillary variables for the absolute varilables. 
                    # the reason is that absolute function can only be used to set a decision variable equal to absolute value of another.
                    diff_v = self.model.addVar(obj=0, vtype=grb.GRB.CONTINUOUS)
                    self.model.addConstr(diff_v == v - X[dim])
                    abs_v = self.model.addVar(obj=0, vtype=grb.GRB.CONTINUOUS)
                    self.model.addGenConstrAbs(abs_v, diff_v)
            ...
            # the l1 constraint
            self.model.addConstr(grb.quicksum(a for a in abs_vars) <= eps)
        else:
            assert X.dim() == 4
            X = X.squeeze(0)
            abs_vars = []
            for chan in range(X.size(0)):
                chan_vars = []
                chan_lbs = []
                chan_ubs = []
                for row in range(X.size(1)):
                    row_vars = []
                    row_lbs = []
                    row_ubs = []
                    # for col in range(X.size(2)):
                    for col in range(2*X.size(2)):
                        # lb = X[chan, row, col]-eps
                        # ub = X[chan, row, col]+eps
                        lb=0
                        ub=1
                        if not self.model_built:
                            v = self.model.addVar(lb=lb, ub=ub, obj=0,
                            ...
                            # the reason is that absolute function can only be used to set a decision variable equal to absolute value of another.
                            # diff_v = self.model.addVar(obj=0, vtype=grb.GRB.CONTINUOUS)
                            # self.model.addConstr(diff_v == v - X[chan, row, col])
                            # abs_v = self.model.addVar(obj=0, vtype=grb.GRB.CONTINUOUS)
                            # self.model.addGenConstrAbs(abs_v, diff_v)
                            # abs_vars.append(abs_v)
                            ##
                            #### modelling simplex constraint
                            abs_vars.append(v)
            ...
            self.model.addConstr(grb.quicksum(a for a in abs_vars) <= 1)
    ...
    if time_budget is not None:
        ...
        while attempt <= 1:
            ...
            if model.status == grb.GRB.OPTIMAL:
                ...
                # model.feasRelaxS(1, False, True, False)
                # model.optimize()
                # print(model.status)
                raise Exception(f"Unexpected Status code: {model.status}")
    if attempt == 1:
