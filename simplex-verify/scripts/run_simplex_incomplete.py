...
def run_incomplete_verification(gpu, cpus, experiment, epsilon):    ###### 从这里开始执行 ######
    ## 1. run on gpu,    2. implement alg anf run simple crown
    ### 先：一组图片，一个方法
    ...
    if experiment == "cifar_sgd":
        ...
        #          f"python tools/bounding_tools/simplex_cifar_bound_comparison.py " \
        #          f"--network_filename ./data/cifar_sgd_8px.pth --eps {epsilon} --target_directory ./results/sgd8_ll --from_intermediate_bounds"
        # taskset命令用于指定程序应在哪些 PU上运行。这不是由Python中的argument parser直接处理的；它是在Python环境之外管理的系统级规范。
...
if __name__ == "__main__":
    ...
    #command = "python run_simplex_incomplete.py"
    #os.system(command)
    ...
    # Computing bounds      对不同的image set测试
