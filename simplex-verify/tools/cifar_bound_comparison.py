...
def make_elided_models(model, return_error=False):  # 给model换最后一个layer
    ...
    nb_classes = last_layer.out_features    # 存最后一个layer的featur

    for gt in range(nb_classes):
        new_layer = nn.Linear(last_layer.in_features,
                              last_layer.out_features-1)    # 对每个feature生成一个layer
...
def read_bounds(target_file, to_ignore=None):
    with open(target_file, 'r') as res_file:
        Lines = res_file.readlines()
        # bounds = re.split(r'\t+', Lines[1])
        bounds = Lines[1].split('\t')
        bounds = list(map(float, bounds))
        
    return Lines[0], bounds
